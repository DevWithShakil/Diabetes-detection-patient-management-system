    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden" style="background: linear-gradient(135deg, #0f172a 0%, #334155 100%);">
        <div class="card-body p-4 d-flex align-items-center justify-content-between text-white">
            ...
            @php
                ...
                // ✅ CHECK: Doctor Approved or Not?
                $isApproved = $nextAppointment && $nextAppointment->status === 'approved';
            @endphp

            <div class="d-none d-md-block text-end">
                <small class="text-white-50 d-block text-uppercase fw-bold mb-1" style="font-size: 10px;">Medical Status</small>

                @if($isApproved)
                    {{-- SHOW RESULT ONLY IF APPROVED --}}
                    @if($finalPrediction === 1)
                        <span class="badge bg-danger bg-opacity-25 border border-danger text-white px-3 py-2 rounded-pill">
                            <i class="fa-solid fa-triangle-exclamation me-1"></i> Diabetic
                        </span>
                    @elseif($finalPrediction === 0)
                        <span class="badge bg-success bg-opacity-25 border border-success text-white px-3 py-2 rounded-pill">
                            <i class="fa-solid fa-shield-heart me-1"></i> Non-Diabetic
                        </span>
                    @else
                        <span class="badge bg-secondary bg-opacity-25 border border-secondary text-white px-3 py-2 rounded-pill">
                            Result Pending
                        </span>
                    @endif
                @else
                    {{-- HIDE RESULT IF PENDING --}}
                    <span class="badge bg-warning bg-opacity-25 border border-warning text-white px-3 py-2 rounded-pill">
                        <i class="fa-solid fa-user-doctor me-1"></i> Under Doctor Review
                    </span>
                @endif
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 h-100">
                ...
                <div class="card-body p-4">
                    @if($nextAppointment)
                        ...
                        @if($latestNote)
                            <div class="p-3 bg-light rounded-3 border border-light-subtle mt-3">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="bg-white p-1 rounded-circle border d-flex align-items-center justify-content-center" style="width: 24px; height: 24px;">
                                            <i class="fa-solid fa-user-doctor text-primary" style="font-size: 12px;"></i>
                                        </div>
                                ...
                                <p class="small text-secondary mb-0 ps-1" style="line-height: 1.5;">
                                    {{ $latestNote->note }}
                                </p>
                            </div>
                        @else
                            <div class="text-muted small fst-italic mt-3">
                                <i class="fa-regular fa-comment-dots me-1"></i> No notes from doctor yet.
                            </div>
                        @endif
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-grid gap-2">
                        <a href="{{ route('patient.simpletest') }}" class="btn btn-light border text-start px-3 py-2 d-flex align-items-center gap-3 hover-shadow">
                            <div class="bg-white border rounded-circle d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                                <i class="fa-solid fa-flask text-primary"></i>
                            </div>
                            <div>
                                <div class="fw-bold text-dark small">Update Health Data</div>
                                <div class="text-muted" style="font-size: 10px;">Input latest vitals for review</div>
                            </div>
                        </a>
                        ...
                        <a href="{{ route('patient.appointments.index') }}" class="btn btn-light border text-start px-3 py-2 d-flex align-items-center gap-3 hover-shadow">
                            ...
                            <div>
                                <div class="fw-bold text-dark small">View All Appointments</div>
                                <div class="text-muted" style="font-size: 10px;">Check history and status</div>
                            </div>
                        </a>
                    </div>
